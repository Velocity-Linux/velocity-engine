...
impl Optimizer for IoOptimizer {
    async fn apply(&mut self, profile: &ProfileConfig, pids: &[u32], state: &mut OptimizationState) -> Result<()> {
        if let (Some(io_class), Some(priority)) = (&profile.io_priority, profile.process_priority) {
            for pid in pids {
                ...
                let current_priority = if current_priority == -1 && std::io::Error::last_os_error().raw_os_error() == Some(0) {
                    0
                } else {
                    current_priority
                };
                ...
                if current_priority != priority {
                    crate::system::SystemUtils::set_process_priority(*pid, priority)
                ...
                crate::system::SystemUtils::set_io_priority(*pid, io_class, priority)
    ...
    async fn restore(&self, state: &OptimizationState) -> Result<()> {
        for (pid, priority) in &state.process_priorities {
            if let Err(e) = crate::system::SystemUtils::set_process_priority(*pid, *priority) {
                warn!("Cannot restore priority for PID {}: {}", pid, e);
            }
        }

        for (pid, (class, priority)) in &state.io_priorities {
            if let Err(e) = crate::system::SystemUtils::set_io_priority(*pid, class, *priority) {
